...
const CarDetailScreen = ({ car, onFavoritePress, onViewUpdate, setSelectedCar }: any) => {
	console.log("car")
	if (!car) return null
  console.log(car)
  ...
  const handleWhatsApp = useCallback(() => {
    if (car.dealership_phone) {
      ...
      const url = `https://wa.me/${car.dealership_phone}?text=${encodeURIComponent(
        message
      )}`
      Linking.openURL(url)
    } else {
  ...
  return (
    <View 
      className={`${isDarkMode ? 'bg-black' : 'bg-white'} flex-1`}
     >
      <TouchableOpacity 
        onPress={() => router.back()}
