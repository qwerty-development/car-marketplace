...
/**
 * First step of secure email change flow.
 * Verifies the OTP sent to the CURRENT email address.
 * After verification, user proceeds to verify the NEW email.
 */
export default function VerifyCurrentEmailOtpScreen() {
  ...
  const currentEmail = params.currentEmail || "";
  ...
  const handleVerify = async (otpCode?: string) => {
    ...
    try {
      // Verify OTP for CURRENT email with type 'email_change'
      ...
      // Now proceed to verify the NEW email
      Alert.alert(
        t("email.current_email_verified_title"),
        t("email.current_email_verified_message"),
        [
          {
      ...
      console.error("[VerifyCurrentEmailOtp] Error verifying OTP:", err);
  ...
  const handleResendCode = async () => {
    ...
    try {
      // Resending triggers a new email change request
      ...
      Alert.alert(t("email.code_sent_title"), t("email.codes_resent_both"));
      clearOtpInputs();
      setResendCooldown(60); // 60 second cooldown
    } catch (err) {
      console.error("[VerifyCurrentEmailOtp] Error resending code:", err);
        ...
        >
          ...
          <View className="flex-row items-center justify-center mb-6">
            <View className="flex-row items-center">
              ...
              <View className={`h-1 w-8 ${isDarkMode ? "bg-neutral-700" : "bg-gray-300"}`} />
              <View className={`w-8 h-8 rounded-full ${isDarkMode ? "bg-neutral-700" : "bg-gray-300"} items-center justify-center`}>
                <Text className={isDarkMode ? "text-white/60" : "text-gray-400"}>2</Text>
              </View>
            </View>
          </View>
          ...
          >
            {t("email.verify_current_email_subtitle")}
          </Text>
          <Text
          ...
          {/* Info box about two-step process */}
          <View
            className={`${
              isDarkMode ? "bg-orange-900/30" : "bg-orange-50"
            } p-3 rounded-xl mb-6 flex-row items-start`}
          >
            <Ionicons
            ...
            <Text
              ...
              style={{ textAlign: isRTL ? "right" : "left" }}
            >
              {t("email.two_step_verification_info")}
            </Text>
          </View>
