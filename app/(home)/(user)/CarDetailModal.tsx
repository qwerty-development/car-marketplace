...
export default function CarDetailModal() {
	...
	const [isCurrentlyFavorite, setIsCurrentlyFavorite] =
	...
	const trackCarView = useCallback(
		async (carId: number, userId: string) => {
			try {
				...
				if (data) {
					// Update the car's view count in the parent component
					router.setParams({ viewCount: data.toString() })
				}
			} catch (error) {
	...
	}, [car, user, debouncedTrackCarView])
	...
	const handleFavoritePress = useCallback(async () => {
		const newLikesCount = await toggleFavorite(car.id)
		setIsCurrentlyFavorite(!isCurrentlyFavorite)
		// Update the car's like count in the car object
		car.likes = newLikesCount
	}, [car.id, toggleFavorite, isCurrentlyFavorite])
		...
		<LinearGradient
			...
			<View
				className={`absolute bottom-0 w-full flex-row justify-around py-2 border-t ${
					isDarkMode
						? 'bg-night border-gray-800'
						: 'bg-light-background border-gray-200'
				}`}>
				{car.dealership_phone && (
					<TouchableOpacity
