...
const OptimizedImage = memo(({ source, style, onLoad }) => {
  ...
  return (
    <View style={style}>
      {!loaded && (
        <View style={[style, { backgroundColor: '#E0E0E0', position: 'absolute' }]} />
      )}
      <Animated.Image
        source={source}
        style={style}
...
const CarDetailModal = memo(({
  ...
  const trackCarView = useCallback(async (carId: any, userId: any) => {
    try {
          ...
          <ScrollView ref={scrollViewRef} style={styles.scrollView}>
            ...
            <View className="p-4 mb-24">
              ...
              <View className="flex-row justify-between mt-4 mb-4">
                <Text className={`text-l ${isDarkMode ? 'text-white' : 'text-black'}`}>
                  Views: {car.views || 0}
                </Text>
                <Text className={`text-l ${isDarkMode ? 'text-white' : 'text-black'}`}>
                  Likes: {car.likes || 0}
              ...
              <Text className={`text-s mt-4 ${isDarkMode ? 'text-white' : 'text-black'} font-bold text-l mb-3`}>
                Description
              </Text>
              <Text className={`text-s mb-4 font-light ${isDarkMode ? 'text-white' : 'text-black'}`}>
                {car.description}
              </Text>
              ...
              <Text className={`text-l mt-4 ${isDarkMode ? 'text-white' : 'text-black'} mb-3 font-bold`}>
                Technical Data
              </Text>
              <View className={`mb-6 mt-3 border ${isDarkMode ? 'border-white' : 'border-black'} rounded-lg`}>
                {[
                  ...
                  { label: 'Drive Train', value: car.drivetrain }
					...
					<View key={index} className={`flex-row p-2 ${index !== 4 ? `border-b ${isDarkMode ? 'border-white' : 'border-black'}` : ''} justify-between py-2`}>
					  <Text className={`text-l ${isDarkMode ? 'text-white' : 'text-black'} font-bold`}>
						{item.label}
					  </Text>
				...
				<TouchableOpacity
				  className={`flex-row items-center justify-center ${isDarkMode ? 'bg-gray-800' : 'bg-gray-200'} p-3 rounded-lg mb-6`}
				  onPress={() => onFavoritePress(car.id)}>
				  <Ionicons
					name={isFavorite(car.id) ? 'heart' : 'heart-outline'}
					size={24}
					color={isFavorite(car.id) ? 'red' : isDarkMode ? 'white' : 'black'}
				  />
				  <Text className={`text-lg font-bold ${isDarkMode ? 'text-white' : 'text-black'} ml-3`}>
				...
				<Text className={`text-lg font-bold ${isDarkMode ? 'text-white' : 'text-black'} mt-2 mb-2`}>
				  Dealer Information
				</Text>
				<View className="border-t border-gray-600 pt-4">
				  <View className="items-center">
					...
					<Text
					  className={`text-xl font-bold ${isDarkMode ? 'text-white' : 'text-black'} mb-2 mt-4`}
				...
				<Text className={`text-xl font-bold ${isDarkMode ? 'text-white' : 'text-black'} mt-8 mb-4`}>
				  Similarly Priced Cars
				</Text>
				<FlatList
				  data={similarCars}
				  renderItem={renderCarItem}
				  keyExtractor={(item) => `${item.id}-${item.make}-${item.model}-${Math.random()}`}
				  horizontal
				  showsHorizontalScrollIndicator={false}
				/>
