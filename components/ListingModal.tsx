...
export const EnhancedColorSelector = memo(
  ({ value, onChange, isDarkMode }: any) => {
    return (
      <ScrollView
        horizontal
        showsHorizontalScrollIndicator={false}
...
const isEmoji = (str: string) =>
  // catches flags & other pictographs
  typeof str === "string" && /\p{Extended_Pictographic}/u.test(str);
  ...
  ({
    ...
    <TouchableOpacity onPress={onSelect} className="mr-3 mb-3 w-28 h-28">
      ...
      {  isEmoji? (
            ...
            <Text style={{ fontSize: 32 }} className="mb-2">
              {icon}
            </Text>
          ) : (
            <MaterialCommunityIcons
              name={icon}
              size={24}
              color={isSelected || isDarkMode ? "#fff" : "#000"}
